...
난수란 무엇인가? 많은 고민이 필요한 철학적 난제라고 생각한다. 당연히 표준을 정해야하기에 NIST SP800-22, 본 문서에서는 질문에 대해서 다음과 같은 대답을 내놓았다. Binary sequence에 대하여 통계학적으로 0과 1이 나올 확률이 똑같이 \\(1/2\\) 이어야한다. 여기서 의문이 생긴다. 그렇다면 만약, 완벽한 난수라면, 완벽히 0과 1이 반반씩 나오는 것이고, 해당 bit stream의 총 길이를 안다면 최소한 길이가 홀수일 때, 다음 bit가 0일지 1일지 알 수 있다는 의미로 들린다. ~~아마, 이는 내가 통계학적 지식이 많이 부족하기 때문일지도 모른다.~~

본 문서의 겨우 서론을 읽는데 엄청난 시간이 들었다. 필자는 Computer Science를 전공한 학생으로, 통계학적 지식이 당연히 통계학과에 비해서 많이 부족하다. 통계학계에서 두루 쓰이는 용어 역시도 많이 알아듣지 못해서 많은 시간이 걸렸고, 그 용어를 이해하는데에도 많은 시간이 걸렸기에 이 정도의 시간이 걸렸다고 생각한다. 이 글을 꼼꼼히 잘 읽는다면 Randomness 에 대한 Insight가 생김은 물론이거니와, **통계학적 기본 지식은 충분히 배울 수 있을 것 같다**는 생각이 든다.
...
각각의 테스트는 검정통계량 값(해당 데이터에 대한 함수값)에 근거한다. 만약, 검정통계량이 \\(S\\) 이고, 임계치가 \\(t\\) 라면, 1종 오류의 확률은
\\(P(S>t \\  \Vert \\  H_0 \\  \mathrm{is \\  true})
 = P(\mathrm{reject} \\ H_0 \\  \vert \\  H_0 \\  \mathrm{is \\  true})\\)
이고, 2종 오류의 확률은
\\(P(S \le t \\  \Vert \\  H_0 \\  \mathrm{is \\  false})
 = P(\mathrm{accept} \\  H_0 \\  \vert \\ H_0 \\ \mathrm{is \\  false})\\) 이다. 
검정통계량은 _**P\-value**_ 를 계산하기 위해서 사용되는데, 이  _**P\-value**_ 란, 귀무가설에 대한 근거의 강도를 요약한다. 이 테스트들에 대하여, 각각의  _**P\-value**_ 는 \[어떤 테스트에 의해서 어떤 종류의 랜덤하지 않은지를 평가하는 것을 고려해볼 때, **완벽한 RNG가 테스트된 수열보다 덜 랜덤한 수열을 만들 확률**\] 을 의미한다.[^22] 만약 테스트에서 _**P\-value**_ 가 1이라고 정해지면, 그 수열은 완벽한 난수성을 갖는 것 같다고 한다. 0에 가까운 _**P\-value**_ [^23] 는 수열이 완전히 랜덤이 아닌 것 같다는 것을 알려준다. 테스트에 대해서 유의수준 \\((\alpha)\\) 는 정해질 수 있다. 만약, _**P\-value**_ \\(\ge \alpha\\) 이면, 귀무가설은 채택된다. 즉, 수열은 랜덤하다. _**P\-value**_ \\(< \alpha\\) 이면, 귀무가설은 기각된다. 즉, 수열은 랜덤하지 않다. \\(\alpha\\) 값은 1종 오류를 의미한다. 통상적으로 \\((\alpha)\\) 값은 \\([0.001, 0.01]\\) 사이로 정해진다.

예시

- \\(\alpha = 0.001\\) 은 "만약 어떤 수열이 랜덤할 때, 1000개 중 1개의 수열이 기각될 것으로 기대됨."을 의미한다. \\(\alpha \ge 0.001\\) 일 때, 그 수열이 랜덤하다는 것은 99.9%의 신뢰수준을 가지고 있다. \\(\alpha < 0.001\\) 일 때, 그 수열이 랜덤하지 않다는 것에 대해서 99.9%의 신뢰수준을 가지고 있다.

본 문서에서 \\(\alpha\\) 값은 0.01로 정하였다. 많은 경우에서 **예시에 있는 파라미터들이 권장하는 값들을 따르지 않음**을 주의하라. 예시는 보여주기의 목적일 뿐이다.

### Considerations for Randomness, Unpredictability and Testing

다음의 가정은 테스트 되는 랜덤 이진 수열(Random Binary Sequences)에 대하여 세워졌다.

1. **Uniformity**<br>
Random 이나 Pseudorandom 비트 수열을 만드는 과정 중의 어떠한 지점에서도, 0이나 1의 존재는 거의 같아야한다. 즉, 각각의 확률은 정확히 \\(1/2\\) 이어야 한다. "\\(n = \\) 수열의 길이" 일 때, 0 또는 1의 예상 숫자는 \\(n/2\\)이어야 한다.

2. **Scalability**<br>
어떤 수열에 적당한 어떠한 테스트든 랜덤하게 추출된 부분수열에 대해서 똑같이 적용될 수 있어야한다. 만약 수열이 랜덤하다면, 어떠한 추출된 부분수열도 똑같이 랜덤해야한다. 그러므로, 어떠한 추출된 부분수열도 난수성에 대한 어떠한 테스트도 통과할 수 있어야한다.

3. **Consistency**<br>
Generator의 행동양상은 시작값(Seed)에서부터 일관되어야 한다. 하나의 시드값으로부터의 출력에 대해서 PRNG를 테스트하는 것은 부적절하며, 마찬가지로 하나의 물리현상에서 도출된 출력에 기반한 RNG를 테스트하는 것도 부적절하다.[^24]
...
[^22]: 필자 주 - 말이 어려운데, 쉽게 설명하자면 "완벽한 RNG가 좀 이상한 수열을 생성할 확률" 정도로 해석하면 될 듯하다.

[^23]: [_**P\-value**_ of zero](https://www.researchgate.net/post/p_value_of_0000) 라는 표현은 프로그램상으로 0에 가까운 _**P\-value**_ 값이라고 한다.

[^24]: 필자 주 - 시드값 하나만 보지 말고 여러 개를 보라는 의미이다. "어? 시드값 1로 해서 테스트 해봤는데 난수 아님!" 이런 오류는 범하지 말라는 의미.