...
$$
\LARGE \mathrm{SHA-1}(P||M_1^{(1)}||M_2^{(1)}||S) = \mathrm{SHA-1}(P||M_1^{(2)}||M_2^{(2)}||S)
$$
...
$$
\LARGE m_i = (m_{i-3} \oplus m_{i-8} \oplus m_{i-14} \oplus m_{i-16})^{\circlearrowleft 1},\quad \mathrm{for} \: 16 \leq i < 80.
$$
...
$$
\large A_{i+1} = A_{i}^{\circlearrowleft 5} + \varphi_{i} (A_{i-1}, A_{i-2}^{\circlearrowleft 2}, A_{i-3}^{\circlearrowleft 2}) + A_{i-4}^{\circlearrowleft 2} + K_i + m_i,
$$
...
++
\large CV_{j+1} = (a + A_{80}, b + A_{79}, c + A_{78}^{\circlearrowleft 2}, d + A_{77}^{\circlearrowleft 2}, e + A_{76}^{\circlearrowleft 2}).
++
